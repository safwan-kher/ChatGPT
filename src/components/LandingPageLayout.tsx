...
const LandingPageLayout: FC<{ pageProps: any }> = ({ children, pageProps }) => {
  const router = useRouter();
  const [cookies] = useCookies();
  const locale = router.locale === "en" ? "en" : "de";
  const [ticking, setTicking] = useState(false);
  const context = useContext(AppContext);
  const alternativeLanguagePageSlug = pageProps.alternative_language_page_slug;
  // On scroll stuff
...
const getLandingPageLayout: IGetLayout = (page: ReactElement) => {
  return <LandingPageLayout pageProps={page.props}>{page}</LandingPageLayout>;
};

export { getLandingPageLayout };
